...
function fixed_header(){
  ...
  if ( $logo_selector && KORTEZ.fixed_nav && KORTEZ.fixed_header_logo ) { 
    if ( $mastheadHeight < $(window).scrollTop()){
      if ( KORTEZ.separate_logo == '' ){
        if ( KORTEZ.the_custom_logo !== '' ){
          $logo_selector.src = KORTEZ.the_custom_logo;
        }
      }else{
        $( '.site-header .site-branding img' ).css( 'display' , 'block' );
        if( !KORTEZ.mobile_fixed_nav_off || width >= 782 ){
          $logo_selector.src = KORTEZ.separate_logo;
        }
      }
    }else{
      if ( KORTEZ.header_two_logo !== '' && ( KORTEZ.is_front_page || KORTEZ.overlay_post || KORTEZ.overlay_page ) && KORTEZ.is_header_two ){
         $logo_selector.src = KORTEZ.header_two_logo;
      }else if ( KORTEZ.the_custom_logo !== '' ) {
          $logo_selector.src = KORTEZ.the_custom_logo;
      }else if ( KORTEZ.separate_logo !== '' ){
        $( '.site-header .site-branding img' ).css( 'display' , 'none' );
      }
    }
  }
  if ( $mastheadHeight > $( window ).scrollTop() || $( window ).scrollTop() == 0 ) {
      if ( KORTEZ.fixed_nav && $( '#masthead.site-header' ).hasClass( 'sticky-header' )){
          $( '#masthead.site-header' ).removeClass( 'sticky-header' );
          // Fixed header in admin bar
          if( KORTEZ.is_admin_bar_showing && width >= 782 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
          if( KORTEZ.is_admin_bar_showing && width <= 781 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
      }
  }else if ( KORTEZ.fixed_nav && !$( '#masthead.site-header' ).hasClass( 'sticky-header' )){
    if( !KORTEZ.mobile_fixed_nav_off || width >= 782 ){
      ...
      if( KORTEZ.is_admin_bar_showing && width <= 600 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
      }
      if( KORTEZ.mobile_fixed_nav_off && width <= 781 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
    }
  }
}
...
jQuery( document ).ready( function() {
  ...
  */
  $( '.header-image-slider' ).slick({
      ...
      fade: KORTEZ.header_image_slider.fade,
      speed: parseInt( KORTEZ.header_image_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: KORTEZ.header_image_slider.autoplay,
      autoplaySpeed: KORTEZ.header_image_slider.autoplaySpeed,
      infinite: true,
  ...
  */
  $( '#primary-menu' ).slicknav({
      ...
      label: KORTEZ.responsive_header_menu_text,
...
jQuery( window ).on( 'load', function(){
  ...
  * Back to top
  */
  if( KORTEZ.enable_scroll_top == true && $(window).scrollTop() > 200 ){
...
jQuery( window ).on( 'scroll', function(){
  ...
  * Back to top
  */
  if( KORTEZ.disable_scroll_top == false && $(window).scrollTop() > 200 ){
    $( '#back-to-top' ).fadeIn( 200 );
  } else{
    $( '#back-to-top' ).fadeOut( 200 );
  }
